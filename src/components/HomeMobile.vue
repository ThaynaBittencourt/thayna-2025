...
onMounted(() => {
  ...
  tlLoader
    .to(counter, {
      value: 100,
      duration: 2,
      onUpdate: () => { percentage.value = Math.floor(counter.value); },
    })
    .to('.loader-container', {
      yPercent: -100,
      duration: 1.2,
...
<template>
  <div class="noise-container min-h-screen overflow-x-hidden bg-[#F9F9F7] text-black selection:bg-black selection:text-white">

    <!-- HERO -->
    <section class="relative flex min-h-screen flex-col items-center justify-center px-6 pt-20 pb-12">

      <!-- Nome (topo) -->
      <div class="w-full">
        <h1 class="reveal-header font-ubermove-bold translate-y-[100px] text-[8vw] leading-[0.85] font-bold tracking-tighter uppercase opacity-0">
          Thaynã <br />
          <span class="text-[10vw] font-bold font-ubermove-bold text-gray-400">Bittencourt</span>
        </h1>
      ...
      <div class="reveal-header my-8 w-full translate-y-[60px] opacity-0">
        <div class="hero-img-wrapper aspect-[4/4] w-full overflow-hidden rounded-2xl bg-zinc-200">
          <img
        ...
        <div class="pb-5 pt-3 justify-center flex items-center overflow-hidden">
          <span class="reveal-header inline-block translate-y-[60px] text-[10px] font-bold tracking-widest font-ubermove text-emerald-600 uppercase opacity-0">
      ...
      <div class="w-full py-10">
        <p class="reveal-header font-ubermove translate-y-[60px] text-base text-zinc-600 opacity-0">
        ...
        <div class="reveal-header pt-6 flex translate-y-[60px] gap-5 opacity-0">
          <a href="https://github.com/ThaynaBittencourt/" target="_blank">
            <Github class="h-5 w-5 cursor-pointer transition-colors hover:text-emerald-600" />
          </a>
          <a href="https://www.linkedin.com/in/thayn%C3%A3-bittencourt-470571274/" target="_blank">
            <Linkedin class="h-5 w-5 cursor-pointer transition-colors hover:text-emerald-600" />
          </a>
          <a href="mailto:professornegreco@gmail.com">
            <Mail class="h-5 w-5 cursor-pointer transition-colors hover:text-emerald-600" />
          </a>
        </div>
      </div>
    ...
    <div class="overflow-hidden">
      <section class="div-horizontal flex h-screen w-[300vw] flex-nowrap bg-black text-white">
        <div class="secao-horizontal flex h-full w-screen flex-shrink-0 items-center justify-center border-r border-white/10">
          <h1 class="font-ubermove text-[10vw] font-bold uppercase">Agilidade</h1>
        </div>
        <div class="secao-horizontal flex h-full w-screen flex-shrink-0 items-center justify-center border-r border-white/10 bg-emerald-950">
          <h1 class="font-ubermove text-[10vw] font-bold uppercase">Modernidade</h1>
        </div>
        <div class="secao-horizontal flex h-full w-screen flex-shrink-0 items-center justify-center bg-zinc-900">
    ...
    <section class="bg-black px-6 py-16 text-white">
      ...
      <div class="flex flex-col gap-12">
        <div v-for="(project, idx) in projects" :key="idx" class="group cursor-pointer">
          <div class="image-reveal relative aspect-[3/4] overflow-hidden rounded-xl bg-zinc-900">
            <img
              :src="project.image"
            ...
            />
            <div class="absolute top-4 right-4 rounded-full border border-white/20 bg-black/20 p-3 backdrop-blur-md">
          ...
          <div class="mt-5 flex items-start justify-between">
            ...
            <span class="font-mono text-xs text-zinc-600">{{ project.year }}</span>
          </div>
        </div>
      </div>
    </section>
    ...
    >
      <div class="text-center">
        <p class="font-ubermove mb-8 text-[10px] tracking-[0.8em] uppercase opacity-30">
          Loading Portfolio
        </p>
