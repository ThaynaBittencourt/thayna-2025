...
function iniciarAnimacoesPrincipais() {
  ...
  // Aguarda o layout estabilizar antes de criar o ScrollTrigger
  setTimeout(() => {
    ...
    if (slider && innerSlider && sections.length > 0) {
      gsap.timeline({
        scrollTrigger: {
          ...
          anticipatePin: 1,
          onRefresh: (self) => self.scroll(self.scroll()), // força recalculo correto
        },
      }).to(innerSlider, {
        x: () => -(innerSlider.scrollWidth - window.innerWidth),
        ease: 'none',
      });

      ScrollTrigger.refresh();
    }
  }, 300); // delay para garantir que o DOM e fonts carregaram
